const myContainer = <HTMLDivElement>document.querySelector("#container");
const toDoForm = <HTMLFormElement>document.querySelector("#to-do-form");
const toDOInput = <HTMLInputElement>document.querySelector("#to-do-input");
const toDoList = <HTMLDivElement>document.querySelector("#to-do-list");
...
let myList: ToDoItems[] = getListFromLocalStorage()
// !== null ? getListFromLocalStorage() : toDo;
...
function showToDo(items: ToDoItems[]): void {
    toDoList.innerHTML = "";
...
function checkItem() {
    myList.forEach((item) => {
        if (item.isDoing) {
            const didItem = <HTMLDivElement>document.querySelector(`div[data-id="${item.id}"]`);
...
function saveListToLocalStorage(list: ToDoItems[]) {
    localStorage.setItem("To Do List", JSON.stringify(list));
}

function getListFromLocalStorage(): ToDoItems[] {
    try {
        const data = localStorage.getItem("To Do List")
        return data ? JSON.parse(data) as ToDoItems[] : toDo;
    } catch (error) {
        return []
    }
}
